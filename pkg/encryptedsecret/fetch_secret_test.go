...
func TestExecCommandError(t *testing.T) {
	...
	_, err = execCommand(inputPayload)
	require.NotNil(t, err)
	assert.Equal(t, "Error while running './test/response_too_long.sh': command output was too long: exceeded 20 byte", err.Error())
}
...
func TestFetchSecretMissingSecret(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.Equal(t, "secret handle 'handle1' was not decrpyted by the secret_backend_command", err.Error())
}
...
func TestFetchSecretErrorForHandle(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.Equal(t, "An error occurs while decrpyting 'handle1': some error", err.Error())
}
...
func TestFetchSecretEmptyValue(t *testing.T) {
	...
	_, err := fetchSecret([]string{"handle1"})
	assert.NotNil(t, err)
	assert.Equal(t, "error: decrpyted password for 'handle1' is empty", err.Error())
